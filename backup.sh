...
clear
echo " VPS Data Backup By d0x-dev "
sleep 1
...
cd /root
zip -r $IP-$date-$domain-Infinite-SSH.zip backup > /dev/null 2>&1
rclone copy /root/$IP-$date-$domain-Infinite-SSH.zip dr:backup/
url=$(rclone link dr:backup/$IP-$date-$domain-Infinite-SSH.zip)
id=(`echo $url | grep '^https' | cut -d'=' -f2`)
link="https://drive.google.com/u/4/uc?id=${id}&export=download"
clear
echo -e "\033[1;37mVPS Data Backup By d0x-dev\033[0m
\033[1;37mTelegram : https://t.me/todfix667 / @d0x-dev\033[0m"
echo ""
...
rm -rf /root/backup
rm -r /root/$IP-$date-$domain-Infinite-SSH.zip
echo ""
